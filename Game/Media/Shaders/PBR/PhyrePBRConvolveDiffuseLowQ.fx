...
static float4 GetDiffuseLowQMip0Col(float3 dir)
{
...
float4 ConvolveDiffuseLowQPS(PrefilterLightprobeOut IN) : FRAG_OUTPUT_COLOR0
{
	return GetDiffuseLowQMip0Col(normalize(IN.Direction));
}

technique11 PBRConvolveDiffuseLowQ
{
	pass pass0
	{
		SetVertexShader( CompileShader( vs_4_0, ConvolveVS() ) );
		SetPixelShader( CompileShader( ps_4_0, ConvolveDiffuseLowQPS() ) );
