...
Texture2D <float4> lightprobe;								// The lightprobe texture to pre-filter.
...
int InvertedFaces;											// The flag that indicates that the faces are inverted.
...
void doFace(inout SH_ORDER_STRUCT4 results, CubemapNormalGenerator cubemapNormalGenerator, CubemapFacePermutor cubemapFacePermutor, uint size, uint faceId, uint invertedFacesMask, uint2 posInTile)
{
	uint offsetY = invertedFacesMask ? ((5-faceId) * size) : faceId * size;
	offsetY |= invertedFacesMask;				// OR in the vertical face flip.  We XOR the offsetY to both select the face and perform the optional face flip.
	...
	for (uint y=0; y<size; y+=8)
	{
		for (uint x=0; x<size; x+=8)
		{
			...
			// Get cubemap texel.
			float4 rgba = lightprobe.Load(int3(x+posInTile.x, (y+posInTile.y)^offsetY, 0));			// Add in this sample.
...
[numthreads(8, 8, 2)]
void CS_GenerateSphericalHarmonicCoefficientsStacked(uint3 GroupThreadId : SV_GroupThreadID)
{
	...
	// The invertedFacesMask is XORed into the y coordinate to vertically flip the faces.
	uint invertedFacesMask = InvertedFaces ? size-1 : 0;
	...
	doFace(results, cubemapNormalGenerator, permutorX, size, 0+GroupThreadId.z, invertedFacesMask, posInTile);

	CubemapFacePermutor permutorY = CreatePermutor(2+GroupThreadId.z);
	doFace(results, cubemapNormalGenerator, permutorY, size, 2+GroupThreadId.z, invertedFacesMask, posInTile);

	CubemapFacePermutor permutorZ = CreatePermutor(4+GroupThreadId.z);
	doFace(results, cubemapNormalGenerator, permutorZ, size, 4+GroupThreadId.z, invertedFacesMask, posInTile);
...
technique11 GenerateSphericalHarmonicCoefficientsStacked
{
	pass p0
	{
		SetComputeShader(CompileShader(cs_5_0, CS_GenerateSphericalHarmonicCoefficientsStacked()));
	}
}
