...
{
	PEmitLinePositionVelocityLifetimeStateStruct	emissionState;								// The emission state for emitting line positions, velocities and lifetimes.
};

[numthreads(64, 1, 1)]
void CS_EmitLinePositionVelocityLifetime(uint3 DispatchThreadId : SV_DispatchThreadID)
{
	...
	if (particleIndex < topEndOfEmissionWindow)
	{
		...
		float r2 = genRandFloat(vRandSeed.x);
		...
		float varianceDirectionX = interpSpread(0.0f, emissionState.m_emitterDirectionVariance, r1.z);
		float varianceDirectionY = interpSpread(0.0f, emissionState.m_emitterDirectionVariance, r1.w);
		float3 linePosLocal = interpSpread(emissionState.m_emitterLineStart, emissionState.m_emitterLineEnd, r2.xxx);
		float3 linePos = emissionState.m_parentFacingX * linePosLocal.x +
						 emissionState.m_parentFacingY * linePosLocal.y +
						 emissionState.m_parentFacingZ * linePosLocal.z;

		float3x3 randDirectionMatrixX = rotationX(varianceDirectionX);
		float3x3 randDirectionMatrixZ = rotationZ(varianceDirectionY);
		randDirectionMatrixX = mul(randDirectionMatrixX, randDirectionMatrixZ);

		float3 directionVariance = mul(randDirectionMatrixX, emitterDirection);

		float3 emittedLocation = linePos + emitterPoint;
...
technique11 EmitLinePositionVelocityLifetime
{
	pass p0
	{
		SetComputeShader( CompileShader( cs_5_0, CS_EmitLinePositionVelocityLifetime() ) );
	}
};
