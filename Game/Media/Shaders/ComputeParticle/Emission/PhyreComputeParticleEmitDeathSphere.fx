...
{
	PEmitDeathSphereStateStruct						emissionState;							// The emission state for emitting particles from dead particles.
};

[numthreads(64, 1, 1)]
void CS_EmitDeathSphere(uint3 DispatchThreadId : SV_DispatchThreadID)
{
	...
	if ((particleIndex < topEndOfEmissionWindow) && (deadParticleIndex < deadState.m_deadPopulation))
	{
		...
		float3 emittedLocation = (vec * radius) + deadParticle_PositionVelocityLifetime[deadParticleIndex].m_location;
...
technique11 EmitDeathSphere
{
	pass p0
	{
		SetComputeShader( CompileShader( cs_5_0, CS_EmitDeathSphere() ) );
	}
};
