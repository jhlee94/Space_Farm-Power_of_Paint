...
RWStructuredBuffer<PStruct48>					particle_state;					// The structure buffer containing 48 byte particle state.
StructuredBuffer<uint2>							particleMoves;					// The mappings for the particles to move.

[numthreads(64, 1, 1)]
void CS_DefragRemap48(uint3 DispatchThreadId : SV_DispatchThreadID)
{
	...
	if (moveIndex < moveCount)
	{
		...
		float4 el3 = particle_state[srceIndex].m_el3;
		particle_state[destIndex].m_el1 = el1;
		particle_state[destIndex].m_el2 = el2;
		particle_state[destIndex].m_el3 = el3;
	}
}

#ifndef __ORBIS__

technique11 DefragRemap48
{
	pass p0
	{
		SetComputeShader( CompileShader( cs_5_0, CS_DefragRemap48() ) );
	}
};
